...
export const seasonStatsApi = {
  list(playerId) {
    ...
    for (const stat of source) {
      ...
      agg.hitting.doubles     += safeN(h.doubles)
      agg.hitting.triples     += safeN(h.triples)
      ...
      agg.pitching.wins         += safeN(p.wins)
      agg.pitching.losses       += safeN(p.losses)
      agg.pitching.saves        += safeN(p.saves)
